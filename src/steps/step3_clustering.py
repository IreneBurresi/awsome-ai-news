...
AI_KEYWORDS = {
    ...
    "learning to learn",
    "meta-learning",
    # Important AI Figures (50+ names)
    "altman",  # Sam Altman (OpenAI)
    "hassabis",  # Demis Hassabis (Google DeepMind)
    "lecun",  # Yann LeCun (Meta AI)
    "hinton",  # Geoffrey Hinton
    "bengio",  # Yoshua Bengio
    "andrew ng",
    "fei-fei li",
    "sutskever",  # Ilya Sutskever (OpenAI)
    "karpathy",  # Andrej Karpathy
    "amodei",  # Dario & Daniela Amodei (Anthropic)
    "brockman",  # Greg Brockman (OpenAI)
    "musk",  # Elon Musk (xAI)
    "huang",  # Jensen Huang (NVIDIA)
    "nadella",  # Satya Nadella (Microsoft)
    "pichai",  # Sundar Pichai (Google)
    "zuckerberg",  # Mark Zuckerberg (Meta)
    "suleyman",  # Mustafa Suleyman (Microsoft AI)
    "legg",  # Shane Legg (DeepMind)
    "fridman",  # Lex Fridman
    "stuart russell",
    "tegmark",  # Max Tegmark
    ...
    "gebru",  # Timnit Gebru
    "margaret mitchell",
    "buolamwini",  # Joy Buolamwini
    "breazeal",  # Cynthia Breazeal
    "rodney brooks",
    "thrun",  # Sebastian Thrun
    "lattner",  # Chris Lattner
    "jeremy howard",
    "rachel thomas",
    "zaremba",  # Wojciech Zaremba (OpenAI)
    "schulman",  # John Schulman (OpenAI)
    "radford",  # Alec Radford (OpenAI)
    "jeff dean",
    "vinyals",  # Oriol Vinyals (DeepMind)
    "goodfellow",  # Ian Goodfellow (GAN)
    ...
    "christopher manning",
    "emily bender",
    "floridi",  # Luciano Floridi (Oxford)
    "kate crawford",
    "whittaker",  # Meredith Whittaker
    "gary marcus",
    "chollet",  # François Chollet (Keras)
    "mostaque",  # Emad Mostaque (Stability AI)
    "holz",  # David Holz (Midjourney)
    "delangue",  # Clement Delangue (Hugging Face)
    "mensch",  # Arthur Mensch (Mistral AI)
    "shazeer",  # Noam Shazeer (Character.AI)
    "de freitas",  # Daniel De Freitas
    "gomez",  # Aidan Gomez (Cohere)
    "frosst",  # Nick Frosst (Cohere)
    "yi tay",
    "kanjun qiu",
    "albrecht",  # Josh Albrecht
    "luan",  # David Luan (Adept)
    "vaswani",  # Ashish Vaswani (Transformer)
    "schmidhuber",  # Jürgen Schmidhuber
    "kaplan",  # Jared Kaplan
    "bommasani",  # Rishi Bommasani
    "socher",  # Richard Socher
    "koh",  # Pang Wei Koh
    "liang",  # Percy Liang (già menzionato come percy liang)
}
...
async def _call_gemini_clustering(
    ...
    prompt = prompt_loader.format_prompt(
        ...
        articles_formatted=_format_articles_for_prompt(articles_data),
    ...
    response = client.models.generate_content(
        ...
        config={
            ...
            "response_json_schema": GeminiClusteringResponse.model_json_schema(),
        },
    )
